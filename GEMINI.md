...
- `import { type UploadFile } from "make-api-request-js";` for binary data
...
- `import { BinaryResponse } from "make-api-request-js";` for binary data
...
- Functions should take at max two inputs. The first is a type for input data, and the second is `opts?: RequestOptions` (`import { RequestOptions} from  "make-api-request-js";`)
...
- BEFORE EDITING ANY CODE, CHECK IF THE FILE IS EDITABLE IN [.sdk.json](./.sdk.json)
- Do not remove or change the versions of [make-api-request-js](https://www.npmjs.com/package/make-api-request-js)
- Any imports that are node or browser dependent must be imported using the `require` keyword inline according to the runtime from (`import { RUNTIME } from "make-api-request-js";`)
- Reference the [root readme](./README.md) to understand the module structure of the repo when gathering context. Follow the README links to view parameter tables, example snippets, and response examples.
...
- Always use the mock server Environment enum in tests (`Environment.MockServer`)
- When adding to an existing resource's README.md, add new content <!-- CUSTOM DOCS START --> and <!-- CUSTOM DOCS END --> comment so that the code generator retains the content.
